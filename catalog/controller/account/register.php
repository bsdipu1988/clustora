...
class ControllerAccountRegister extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$mobileNumber=$this->customer->getTelephone();
			
			$name=$this->customer->getFirstName();
			//$totalcost=$this->session->data['totals']['value'];
			//$order=$this->session->data['order_id'];
			$uid=$this->customer->getEmail();$psd=$this->request->post['password'];
			
			$message="Hi ".$name." Your account userid=".$uid." and password=".$psd." FREE adv FREE delivery Any Query call 8908408667 clustora.com";
			$senderId="CLUSTO";
			$routeId="1";
